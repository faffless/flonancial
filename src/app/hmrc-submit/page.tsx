...
function getSuccessMessage(status?: string) {
  if (status === "submitted") {
    return "Amended figures submitted to HMRC successfully.";
  }
...
function getConfirmMessage(status?: string) {
  if (status === "submitted") {
    return "Submit amended cumulative figures for this HMRC-linked period now?";
  }
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-3 text-sm leading-6 text-white/70">
                {getIntroText(data?.update?.status)}
              </p>
            </div>
          ...
          {loading ? (
            <p className="mt-6 text-sm text-white/70">
              Loading HMRC submission preview...
            </p>
          ) : errorMessage ? (
            ...
            <>
              <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <div className="rounded-xl border border-white/10 bg-black/20 p-4">
                  ...
                  <p className="mt-2 text-sm text-white">
                    {data.submission_method ?? "Not set"}
                  </p>
                </div>
                ...
                <div className="rounded-xl border border-white/10 bg-black/20 p-4">
                  ...
                  <p className="mt-2 text-sm text-white">
                    {data.business.hmrc_business_id}
                  </p>
                </div>
                ...
                <div className="rounded-xl border border-white/10 bg-black/20 p-4">
                  ...
                  <p className="mt-2 text-sm text-white">
                    {getStatusLabel(data.update.status)}
                  </p>
                </div>
                ...
                <div className="rounded-xl border border-white/10 bg-black/20 p-4">
                  ...
                  <p className="mt-2 text-sm text-white">
                    {data.update.period_key ?? "Not set"}
                  </p>
                </div>
              </div>
              ...
              {data.update.status === "submitted" ? (
                <div className="mt-4 rounded-xl border border-emerald-400/25 bg-emerald-400/10 p-4">
                  <p className="text-sm text-emerald-200">
                    This period was already submitted once. You can send amended cumulative figures for the same HMRC period.
                  </p>
                </div>
              ) : null}
              ...
              <div className="mt-4 rounded-xl border border-white/10 bg-black/20 p-4">
                <p className="text-[11px] uppercase tracking-[0.18em] text-white/45">
                  HMRC endpoint preview
                </p>
                <p className="mt-2 break-all text-sm text-white/80">
                  {data.hmrc_endpoint ?? "Not set"}
                </p>
                {data.note ? (
                  <p className="mt-2 text-xs text-white/45">{data.note}</p>
                ) : null}
              </div>
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-black/20 p-4">
              <p className="text-sm text-white/80">
                No HMRC submission preview available.
              </p>
            </div>
          )}
        </div>
